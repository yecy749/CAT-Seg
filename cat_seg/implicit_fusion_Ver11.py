...
class ImplicitFusionCATSegVer11(nn.Module):
    @configurable
    ...
    def __init__(
        ...
        *,
        ...
        self.upsample1d = nn.ConvTranspose2d(self.proj_dim, 256, kernel_size=2, stride=2)
        self.upsample2d = nn.ConvTranspose2d(self.proj_dim, 128, kernel_size=4, stride=4)
...
>>>>>>> 51eba470d2bedad3cb2cf38dd5bb06a43452443e
    def forward(self, batched_inputs):
       ...
       #  clip_patch_last_upsample = self.clip_feat_upsample(clip_patch_last_unfold) # use
        ...
        #### ready to add the dino feat. to decoder ####
        dino_res3 = dino_feat[11]
        dino_L7_feat = dino_feat[7][:,1:,:]
        dino_L7_feat = rearrange(dino_L7_feat,"B (H W) C -> B C H W", H=48)
        
        dino_L3_feat = dino_feat[3][:,1:,:]
        dino_L3_feat = rearrange(dino_L3_feat,"B (H W) C -> B C H W", H=48)
        #### ready to add the dino feat. to decoder ####
        # print(dino_L7_feat.shape)
        # print(dino_L3_feat.shape)
        res3 = rearrange(clip_image_features, "B (H W) C -> B C H W", H=24)
        # res4 = rearrange(self.layers[0][1:, :, :], "(H W) B C -> B C H W", H=24) # L3
        # res5 = rearrange(self.layers[1][1:, :, :], "(H W) B C -> B C H W", H=24) # L7
        
        # we change res4 and res5 to swap the feat. used in decoder
        # 这里改了
        res4 = rearrange(self.layers[1][1:, :, :], "(H W) B C -> B C H W", H=24) # L3
        res5 = rearrange(self.layers[0][1:, :, :], "(H W) B C -> B C H W", H=24) # L7
        
        # print(dino_L3_feat.shape)
        # print(dino_L7_feat.shape)
        # print(dino_res5.shape)
        # print(res3.shape)
        # print(res4.shape)
        # print(res5.shape)
        # exit()
