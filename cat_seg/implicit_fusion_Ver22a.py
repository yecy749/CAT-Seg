...
class ImplicitFusionCATSegVer22a(nn.Module):
    @configurable
    ...
    def __init__(
        ...
        *,
        ...
        for l in self.last_layers_indexs:
            self.sem_seg_head.predictor.clip_model.visual.transformer.resblocks[l].register_forward_hook(lambda m, _, o: self.layers.append(o))
    ...
    def forward(self, batched_inputs):
       ...
       #  clip_patch_last_upsample = self.clip_feat_upsample(clip_patch_last_unfold) # use
        ...
        clip_L10 = rearrange(self.layers[2][1:, :, :], "(H W) B C -> B C H W", H=24)
        clip_L11 = rearrange(self.layers[3][1:, :, :], "(H W) B C -> B C H W", H=24)
        # clip_L4_L8_feat = [res4,res5]
        clip_L10_L11_feat = [clip_L10, clip_L11]
        ...
        outputs = self.sem_seg_head(clip_features,dino_feat_down, clip_features_guidance, dino_feat_guidance, clip_L10_L11_feat)
        if self.training:
