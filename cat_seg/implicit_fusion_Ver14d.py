...
class ImplicitFusionCATSegVer14d(nn.Module):
    @configurable
    def __init__(
        self,
        *,
        ...
        self.dense_layer_indexes = [i for i in range(12)]
        self.layers = []
        for l in self.dense_layer_indexes:
    ...
    def forward(self, batched_inputs):
        ...
        clip_intermediate_feat=[]
        for layer in self.layers:
            clip_intermediate_feat.append(rearrange(layer[1:, :, :], "L B C -> B L C"))
        clip_intermediate_feat.append(image_features)

        # clip_intermediate_feat used to guide the encoder and decoder
        ...
        outputs = self.sem_seg_head(clip_features, clip_intermediate_feat)
        if self.training:
