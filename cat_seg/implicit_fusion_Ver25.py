...
class ImplicitFusionCATSegVer25(nn.Module):
    @configurable
    ...
    def forward(self, batched_inputs):
       ...
       #  clip_patch_last_upsample = self.clip_feat_upsample(clip_patch_last_unfold) # use
        ...
        dino_feat_L12 = dino_patch_feat_last_unfold
        ...
        clip_l12 = rearrange(clip_image_features, "B (H W) C -> B C H W", H=24)
        clip_l4  = rearrange(self.layers[0][1:, :, :], "(H W) B C -> B C H W", H=24)
        clip_l8 = rearrange(self.layers[1][1:, :, :], "(H W) B C -> B C H W", H=24)
        clip_features_guidance = [clip_l4,clip_l8,clip_l12]
        ...
        dino_feat_guidance = [dino_feat_L4,dino_feat_L8,dino_feat_L12]
        ...
        outputs, _ = self.sem_seg_head(clip_features,dino_feat_down, clip_features_guidance, dino_feat_guidance)
        
        if self.training:
            targets = torch.stack([x["sem_seg"].to(self.device) for x in batched_inputs], dim=0)
            outputs = F.interpolate(outputs, size=(targets.shape[-2], targets.shape[-1]), mode="bilinear", align_corners=False)
            # fuse_outputs =  F.interpolate(fuse_outputs, size=(targets.shape[-2], targets.shape[-1]), mode="bilinear", align_corners=False)
            num_classes = outputs.shape[1]
            mask = targets != self.sem_seg_head.ignore_value

            outputs = outputs.permute(0,2,3,1)
            # fuse_outputs = fuse_outputs.permute(0,2,3,1)
            ...
            loss1 = F.binary_cross_entropy_with_logits(outputs, _targets)
            # loss2 = F.binary_cross_entropy_with_logits(fuse_outputs, _targets)
           #  losses = {"loss_sem_seg_final" : loss1,"loss_sem_seg_fuse": loss2}
            losses = {"loss_sem_seg_final" : loss1}
            return losses
        
        # 这下面都还没改
        elif self.sliding_window:
            with torch.no_grad():
                # outputs = fuse_outputs # add by ycy
