...
from .FusionAggregatorVer30 import FusionAggregatorVer30
...
class FusionPredictorVer30(nn.Module):
    @configurable
    def __init__(
        self,
        *,
        ...
        elif self.prompt_ensemble_type == "RS":
            prompt_templates = imagenet_templates.RS_TEMPLATES
        else:
        ...
        transformer = FusionAggregatorVer30(
    ...
    def get_text_embeds(self, classnames, templates, clip_model, prompt=None):
        ...
        # exit()
        P = tokens.shape[1]
        tokens = rearrange(tokens,"T P C -> (T P) C")
        class_embeddings = clip_model.encode_text(tokens, prompt)

        class_embeddings = class_embeddings / class_embeddings.norm(dim=-1, keepdim=True)
        class_embeddings = rearrange(class_embeddings,"(T P) C -> T P C", P=P)

        #class_embeddings = class_embeddings.mean(1)
        #class_embeddings = class_embeddings / class_embeddings.norm(dim=-1, keepdim=True)
