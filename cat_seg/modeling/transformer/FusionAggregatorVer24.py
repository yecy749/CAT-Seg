...
class FusionConvDecoder(nn.Module):
    def __init__(self, decoder_dims): # 128 64 32, 64 32 16, 32 16 8
        super().__init__()
        self.clip_proj_L4 = nn.Conv2d(in_channels = 768, out_channels=128, kernel_size=1, stride=1, padding=0)
        self.clip_proj_L8 =  nn.ConvTranspose2d(in_channels= 768, out_channels=64, kernel_size=2, stride=2)
        self.clip_proj_L12 = nn.ConvTranspose2d(in_channels= 512, out_channels=32, kernel_size=4, stride=4)
        self.dino_proj_L4 = nn.Conv2d(in_channels = 768, out_channels=128, kernel_size=2, stride=2, padding=0)
        self.dino_proj_L8 = nn.Conv2d(in_channels = 768, out_channels=64, kernel_size=1, stride=1, padding=0)
        self.dino_proj_L12 = nn.ConvTranspose2d(in_channels = 768, out_channels=32, kernel_size=2, stride=2)
        ...
        self.fuse_head = nn.Conv2d(3*32, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, clip_guidance,dino_guidance):
        ...
        corr_embed,int_1 = self.Fusiondecoder1(corr_embed, self.clip_proj_L4(clip_guidance[0]),self.dino_proj_L4(dino_guidance[0]))
        corr_embed,int_2 = self.Fusiondecoder2(corr_embed, self.clip_proj_L8(clip_guidance[1]),self.dino_proj_L8(dino_guidance[1]))
        corr_embed,int_3 = self.Fusiondecoder3(corr_embed, self.clip_proj_L12(clip_guidance[2]),self.dino_proj_L12(dino_guidance[2]))
...
class DecodFuse(nn.Module):
    ...
    def __init__(self, in_channels): # 128 64 32, 64 32 16, 32 16 8
        ...
        self.CV_branch = nn.Sequential(
            nn.Conv2d(in_channels=in_channels,out_channels=mid_channels_0,kernel_size=3,stride=1,padding=1),
            nn.GroupNorm(mid_channels_0//16, mid_channels_0),
            nn.GELU()
            )
        self.dino_branch = nn.Sequential(
            nn.Conv2d(in_channels=in_channels,out_channels=mid_channels_1,kernel_size=3,stride=1,padding=1),
            nn.GELU())
        ...
        self.start_fuse = nn.Sequential(
            nn.Conv2d(in_channels =in_channels,out_channels=in_channels,stride=1,kernel_size=3,padding=1 ), # 1*1
        ...
        self.dw_4_branch = nn.ModuleList(
            [nn.Conv2d(in_channels =mid_channels_0,out_channels=mid_channels_0,stride=1,kernel_size=2*i-1,padding=(2*i-1)//2,groups=mid_channels_0 ) for i in range(2,6)]
            )
        self.final_fuse = nn.Sequential(
            nn.GroupNorm(in_channels*2//16, in_channels*2),
    ...
    def forward(self, x, clip_guidance,dino_guidance):
        ...
        if clip_guidance is not None:
            ...
            stack_dw_conv_layer = torch.cat(dw_conv_4,dim=1)
            # print(stack_dw_conv_layer.shape)
            out = self.final_fuse(stack_dw_conv_layer)
        return out, intermediate_result
class FusionAggregatorVer24(nn.Module):
    def __init__(self, 
