...
class GenericClassAttentionModuleVer3(nn.Module):
    ...
    def forward(self,x):
        ...
        rearrange_GMP_x = rearrange(GMP_x,'(B T) 1 C ->B T C',T=T)
        rearrange_GAP_x= rearrange(GAP_x,'(B T) 1 C ->B T C',T=T)
        proj_GAP_cls_attn = self.class_attn_MLP(rearrange_GAP_x)
        proj_GMP_cls_attn = self.class_attn_MLP(rearrange_GMP_x)
        cls_attn = proj_GAP_cls_attn + proj_GMP_cls_attn
        cls_attn = self.sigmoid(cls_attn)
        cls_attn = rearrange(cls_attn,'B T 1 -> (B T)')
        ...
        return out_x
class FusionAggregatorVer27(nn.Module):
    def __init__(self, 
        ...
        self.generic_class_attention1 = GenericClassAttentionModuleVer3(hidden_dim)
        self.generic_class_attention2 = GenericClassAttentionModuleVer3(decoder_dims[0])
    ...
    def conv_decoder_with_generic_class_attn(self, x, guidance):
        ...
        corr_embed = rearrange(corr_embed, '(B T) () H W -> B T H W', B=B)
        return corr_embed
    def Fusion_conv_decoer_with_generic_class_attn(self, x, clip_guidance,dino_guidance):
        B = x.shape[0]
        corr_embed = self.generic_class_attention1(rearrange(x,'B C T H W -> B T C H W'))
        corr_embed = rearrange(corr_embed, 'B T C H W -> (B T) C H W')
    ...
    def forward(self, img_feats,dino_feat, text_feats, appearance_guidance,dino_guidance):
        ...
        logit = self.Fusion_conv_decoer_with_generic_class_attn(fused_corr_embed, CLIP_projected_decoder_guidance,DINO_projected_decoder_guidance)
