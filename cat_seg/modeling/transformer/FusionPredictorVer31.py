...
from .FusionAggregatorVer31 import FusionAggregatorVer31
...
class FusionPredictorVer31(nn.Module):
    @configurable
    def __init__(
        self,
        *,
        ...
        if clip_pretrained == "ViT-G" or clip_pretrained == "ViT-H":
            ...
            # transform = Compose([
            #     Resize(384, interpolation=Image.BICUBIC),
            #     CenterCrop(384),
            ...
            #     model_name, 
            #     pretrained=path_to_remote_clip, 
            #     device=device,
            #     )
            ckpt = torch.load(path_to_remote_clip)
            # open_clip_state_dict = ckpt.state_dict()
            
            # message = clip_model.load_state_dict(open_clip_state_dict)
            # clip_model.load_state_dict(open_clip_state_dict,strict = False)
            # exit()
            mapped_state_dict = {}
            ...
            # self.tokenizer = clip.get_tokenizer(model_name)
            # ckpt = torch.load(path_to_remote_clip)
        ...
        transformer = FusionAggregatorVer31(
