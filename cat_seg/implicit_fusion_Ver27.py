...
class ImplicitFusionCATSegVer27(nn.Module):
    @configurable
    ...
    def forward(self, batched_inputs):
       ...
       #  clip_patch_last_upsample = self.clip_feat_upsample(clip_patch_last_unfold) # use
        ...
        dino_feat_L4 = rearrange(dino_feat[7][:,1:,:],"B (H W) C -> B C H W", H=48)
        dino_feat_L8 = rearrange(dino_feat[9][:,1:,:],"B (H W) C -> B C H W", H=48)
